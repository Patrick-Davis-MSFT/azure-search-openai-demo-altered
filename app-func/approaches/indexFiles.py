...
class indexFiles():
    ...
    def run(self, cognitiveSearchIndex, openAIAuth, azure_credential, files): 
        ...
        logging.info("Generating Search Keys..." )
        ...
        for file in files:
            ...
            sys.stdout.flush()
            logging.info("Indexing file: " + str(file))
            try:   
                page_map = self.get_document_text(file, blob_service_client, container_client, azure_credential)
            except Exception as ex:
                logging.error(f"Stage 3: Error getting document text: '{str(ex)}'")
                continue
            use_vectors = True
            sections = self.create_sections(file, page_map, use_vectors)
            self.index_sections(file, sections, azure_credential)
            #Remove File after indexing
            container_client.delete_blob(file)


        logging.info("Stage Completed Indexing...")
    ...
    def create_index(self, azure_credential):
        ...
        try:    
            if search_client.get_index(self.cognitiveSearchIndex):
                ...
                logging.info(f"Index '{self.cognitiveSearchIndex}' already exists, skipping creation")
                return
        except: 
            print(f"\tIndex '{self.cognitiveSearchIndex}' does not exist on '{self.cognitiveSearch}', creating...")
            logging.info(f"Index '{self.cognitiveSearchIndex}' does not exist on '{self.cognitiveSearch}', creating...")
            index = SearchIndex(
            ...
            try:
                ...
                print(str(ex))
                logging.info(f"Error Creating Index: '{str(ex)}'")
                raise (ex)
            finally:
                i=0
        finally:
    ...
    def index_sections(self, filename, sections, azure_credential):
        ...
        logging.info(f"Indexing sections from '{filename}' into search index '{self.cognitiveSearch}'")
    ...
    def compute_embedding(self, text):
        ...
        logging.info(f"Computing embedding for text of length {len(text)}, This will sleep where needed.")
        return openai.Embedding.create(engine=self.embDeployment, model=self.embDeployment, input=text)["data"][0]["embedding"]

    def get_document_text(self, filename: str, blob_service_client: BlobServiceClient, container_client: BlobServiceClient, azure_credential) -> dict:
        offset = 0
        page_map = []
    ...
    def splitPDF(self, staging_cc, search_cc, files) -> None:
        ...
        # get the blob to bytes
            print("Chunking File... " + blob + "\n")
            logging.info("Chunking File: " + blob)
            try:
                blob_client = staging_cc.get_blob_client(blob)
                downloaded_blob = blob_client.download_blob()
                pdf_bytes = downloaded_blob.content_as_bytes()
            except:
                logging.error(f"Error downloading blob: '{blob}'")
                continue
            ...
            if blob.lower().endswith('.pdf'):
                ...
                for i in range(len(pages)):
                    blob_name = blob[:-len(".pdf")] + "-" + str(i) + ".pdf"
                    f = io.BytesIO()
                    writer = PdfWriter()
